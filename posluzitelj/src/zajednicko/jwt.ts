...
export function kreirajToken(korisnik : { korime : string, tip_korisnika_id : number }, 
    tajniKljucJWT : string, jwtValjanost : string) {
    let token = jwt.sign({ korime: korisnik.korime, tip_korisnika: korisnik.tip_korisnika_id }, tajniKljucJWT, { expiresIn: jwtValjanost+"s" });
    return token;
}
